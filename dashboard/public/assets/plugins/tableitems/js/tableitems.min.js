(function($) {
  $.fn.tableitems = function() {
    var $tableitems = this;
    var $addTableItem = $tableitems.find(".addTableItem");
    var $deleteTableItem = $tableitems.find(".deleteTableItem");

    $addTableItem.on("click", function() {
      var $data = $tableitems.find("tbody tr:eq(0)").clone(true).removeClass("d-none").appendTo("#tableitems");
      $data.find("input").val("");
    });

    $deleteTableItem.on("click", function() {
      var $tableTR = $(this).closest("tr");
      var $tableTRCount = $tableitems.find("tbody tr").length;
      if ($tableTRCount > 1) {
        $tableTR.remove();
      }
      else {
        $tableTR.find("input").val("");
      }
    });
  }
}(jQuery));
