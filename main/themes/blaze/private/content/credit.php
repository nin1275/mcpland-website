        ...
        <?php if (get("target") == 'credit'): ?>
          <?php if (get("action") == 'buy'): ?>
            <?php if ($readAccount["email"] != "your@email.com" && $readAccount["email"] != "guncelle@gmail.com"): ?>
              ...
              <?php if ($readSettings["bonusCredit"] != 0): ?>
                ...
                <?php else: ?>
                  <?php echo alertWarning(t__('%bonus%% bonus on purchases over %credit% credits!', ['%credit%' => $readSettings["bonusCreditMinAmount"], '%bonus%' => $readSettings["bonusCredit"]])) ?>
                <?php endif; ?>
              <?php endif; ?>
              <div class="card">
                ...
                <div class="card-body">
                  <form id="buyCreditsForm" action="/apps/main/public/ajax/pay.php" method="post" class="needs-validation" novalidate="">
                    <label class="form-label px-0" for="inputPrice"><?php e__('Amount') ?>:</label>
                    <div class="input-group mb-3">
                      <img class=" position-absolute top-50 start-0 translate-middle-y ms-3" src="<?php echo themePath(); ?>/public/assets/img/icons/coin.svg" width="16">
                      <input name="price" id="inputPrice" class="form-control rounded" type="number" placeholder="<?php e__('Enter the amount.') ?>" required="" min="<?php echo $readSettings["minPay"]; ?>" max="<?php echo $readSettings["maxPay"]; ?>">
                    </div>
  
                    <label for="selectPayment" class="form-label px-0"><?php e__('Payment Method') ?>:</label>
                    <div class="mb-3">
                            ...
                            <option value="<?php echo $readPayment["id"]; ?>">
                              ...
                              <?php if ($readSettings["bonusCredit"] == 0 && $readPayment["bonusCredit"] > 0): ?>
                                <?php if ($readPayment["bonusCreditMinAmount"] == 0): ?>
                                  (<?php e__('%credit%% bonus credits on purchases!', ['%bonus%' => $readPayment["bonusCredit"]]) ?>)
                                <?php else: ?>
                                  (<?php e__('%bonus%% bonus on purchases over %credit% credits!', ['%credit%' => $readPayment["bonusCreditMinAmount"], '%bonus%' => $readPayment["bonusCredit"]]) ?>)
                                <?php endif; ?>
                              <?php endif; ?>
                            </option>
                          <?php endforeach; ?>
                    ...
                    <?php if ($payment->rowCount() > 0): ?>
                      ...
                      ?>
                      <label for="inputFirstName" class="form-label px-0"><?php e__('First Name') ?>:</label>
                      <div class="mb-3">
                      ...
                      <label for="inputLastName" class="form-label px-0"><?php e__('Last Name') ?>:</label>
                      <div class="mb-3">
                      ...
                      <label for="inputEmail" class="form-label px-0"><?php e__('Email') ?>:</label>
                      <div class="mb-3">
                    ...
                    <div class="clearfix">
                      <div class="float-end">
                        <button type="submit" class="btn btn-rounded btn-success" name="buyCredits"><?php e__('Buy Credits') ?></button>
                      </div>
                    </div>
                  </form>
                </div>
            ...
            <div class="card">
              ...
              <div class="card-body">
                <form action="" method="post">
                  <label for="inputUsername" class="form-label px-0"><?php e__('Username') ?>:</label>
                  <div class="mb-3">
                  ...
                  <div class="input-group mb-3">
                    <img class=" position-absolute top-50 start-0 translate-middle-y ms-3" src="<?php echo themePath(); ?>/public/assets/img/icons/coin.svg" width="16">
                    <input name="price" id="inputPrice" class="form-control rounded" type="number" placeholder="<?php e__('Enter the amount.') ?>" required="">
                  </div>
          ...
          <?php elseif (get("action") == 'alert'): ?>
            <?php if (get("result") == 'success'): ?>
              <div class="card mb-3">
                <div class="card-header">
                  <h2 class="card-title"><?php e__('Payment Successful!') ?></h2>
                </div>
            ...
            <?php elseif (get("result") == 'unsuccess'): ?>
              <div class="card mb-3">
                <div class="card-header">
                  <h2 class="card-title"><?php e__('Payment Failed!') ?></h2>
                </div>
                ...
                <?php if ($creditHistory->rowCount() > 0): ?>
                  <div class="card mb-3">
                    <div class="card-header">
                      <div class="row">
                        <div class="col">
                          <h2 class="card-title"><?php e__('Credit Transfer History') ?></h2>
                        </div>
                        <div class="col-auto">
                          <a href="/profile"><?php e__('View All') ?></a>
                        </div>
                      </div>
                    </div>
                ...
                <?php if ($creditHistory->rowCount() > 0): ?>
                  <div class="card mb-3">
                    <div class="card-header">
                      <div class="row">
                        <div class="col">
                          <h2 class="card-title"><?php e__('Credit History') ?></h2>
                        </div>
                        <div class="col-auto">
                          <a href="/profile"><?php e__('View All') ?></a>
                        </div>
                          ...
                          <tbody>
                            <?php foreach ($creditHistory as $readCreditHistory): ?>
                              <tr>
                                ...
                                <td class="text-center">
                                  <?php if ($readCreditHistory["type"] == 1): ?>
                                    <i class="fa fa-mobile" data-bs-toggle="tooltip" data-placement="top" title="<?php echo $readCreditHistory["paymentGatewayName"] ?>"></i>
                                  <?php elseif ($readCreditHistory["type"] == 2): ?>
                                    <i class="fa fa-credit-card" data-bs-toggle="tooltip" data-placement="top" title="<?php echo $readCreditHistory["paymentGatewayName"] ?>"></i>
                                  <?php elseif ($readCreditHistory["type"] == 3): ?>
                                    <i class="fa fa-paper-plane" data-bs-toggle="tooltip" data-placement="top" title="<?php e__('Transfer (Sender)') ?>"></i>
                                  <?php elseif ($readCreditHistory["type"] == 4): ?>
                                    <i class="fa fa-paper-plane" data-bs-toggle="tooltip" data-placement="top" title="<?php e__('Transfer (Receiver)') ?> "></i>
                ...
                <?php if ($topCreditHistory->rowCount() > 0): ?>
                  <div class="card mb-3">
                    <div class="card-header">
                      <div class="row">
                        <div class="col">
                          <h2 class="card-title"><?php e__('Top Donators') ?></h2>
                        </div>
                        <div class="col-auto">
